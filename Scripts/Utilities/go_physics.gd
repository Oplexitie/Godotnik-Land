...
static func cast_ray(world: World2D, origin: Vector2, direction: Vector2, length: float, exclude: Array = [], layer: int = 1) -> Dictionary:
	...
	var overlaps: Array[Dictionary] = space_state.intersect_point(point_params, MAX_RAY_ORIGIN_OVERLAPS)
...
static func overlap_shape(world: World2D, shape: Shape2D, origin: Vector2, rotation: float) -> Array[Dictionary]:
	var space_state: PhysicsDirectSpaceState2D = world.direct_space_state
